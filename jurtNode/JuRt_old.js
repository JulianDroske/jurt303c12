...
function FFmpeg(fn){
	...
	this.init = function(filename){
		...
		if(filename) this.ff = cp.spawn(path.join(modpt,'ffmpeg.exe'),['-loop','1','-i','pipe:','-y','-c:v','mpeg1video','-q','0',filename]);
		else this.ff = cp.spawn(path.join(modpt,'ffplay.exe'),['-fflags','nobuffer','-analyzeduration','1','-i','pipe:']);
...
function XML(content){
	...
	this.load = function(d){
		...
		while(start >= 0){
			...
			this.sKntil('>');
			i++;
			this.tago.push([start, i, hasInner]);
		}
	}
	...
	this.mkObj = function(part, putInner){
		...
		var cont = c.substring(stend[0], stend[1]);
		// var obj = {};
...
function setInterv(cb, time){
	setTimeout(function(){
		if(cb()) setTimeout(arguments.callee, time);
	},time);
}
